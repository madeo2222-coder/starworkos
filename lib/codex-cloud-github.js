...
export function validateCodexGatewayPayload(value) {
  ...
  if (job.provider !== "openai_codex" || job.capability !== "software_development" || !parseRepository(job.repository)) return "UNSUPPORTED_CODEX_GATEWAY_JOB";
  if (!task || typeof task !== "object" || typeof task.title !== "string" || !task.title.trim()) return "INVALID_CODEX_GATEWAY_TASK";
  return null;
}

export function codexIssueContractDigest(issueContract) {
  return createHash("sha256").update(issueContract.title).update("\n").update(issueContract.body).digest("hex");
}
...
export function isTrustedCodexIssue(issue, issueContract, actorLogin) {
  return Boolean(
    issue &&
    !issue.pull_request &&
    issue.user?.login === actorLogin &&
    issue.title === issueContract.title &&
    issue.body === issueContract.body
  );
}

export function findExistingCodexIssue(issues, issueContract, actorLogin) {
  if (!Array.isArray(issues)) return null;
  return issues.find((issue) => isTrustedCodexIssue(issue, issueContract, actorLogin)) ?? null;
}

export function hasCodexDelegationComment(comments, repository, jobId, actorLogin) {
  const expectedBody = buildCodexDelegationComment(repository, jobId);
