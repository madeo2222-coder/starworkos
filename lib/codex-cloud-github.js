...
export function buildCodexDelegationComment(repository) {
  return [
...
export function hasCodexDelegationComment(comments) {
  return Array.isArray(comments) && comments.some((comment) =>
    typeof comment?.body === "string" && /@codex\b/i.test(comment.body)
  );
}
