...
export const DISPATCH_TASK_LIMITS = Object.freeze({
  title: 500,
  content: 12 * 1024,
  priority: 64,
  dueDate: 64,
...
export const DISPATCH_PROTECTED_ACTIONS = Object.freeze([
  "main_merge",
  "production_deploy",
...
export function isAuthorizedDispatchTrigger(env = process.env, suppliedToken) {
  const triggerToken = env.EXTERNAL_AGENT_DISPATCH_TRIGGER_TOKEN;
  return hasMinimumTokenLength(triggerToken) && safeTokenEquals(triggerToken, suppliedToken);
}
...
export function dispatchPayload(job, task, callbackUrl) {
  return {
    job: {
      id: job.id,
      taskId: job.task_id,
    ...
    task: {
      title: boundedString(task?.title, DISPATCH_TASK_LIMITS.title) ?? "",
      content: boundedString(task?.content, DISPATCH_TASK_LIMITS.content),
      priority: boundedString(task?.priority, DISPATCH_TASK_LIMITS.priority),
      dueDate: boundedString(task?.due_date, DISPATCH_TASK_LIMITS.dueDate),
    },
    executionPolicy: {
      protectedActionsRequireHumanApproval: [...DISPATCH_PROTECTED_ACTIONS],
